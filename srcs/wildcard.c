...
/*   Updated: 2021/01/30 10:47:24 by asaadi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int match(char *pattern, char *string, int p, int s)
{
	...
	while (p < (int)ft_strlen(pattern))
	{
		//&& s < ft_strlen(string))
		//'*' match '.' if it's not the first char
		if (pattern[p] == '*'&& (string[s] != '.' || s != 0))
		{
			if (s < (int)ft_strlen(string) && (ret = match(pattern, string, p, s+1)))
				return ret;
			else
				return (match(pattern, string, p+1, s));
		}
		else if (s < (int)ft_strlen(string) && pattern[p] == string[s])
		{ 
			p++;
			s++;
		}
	...
	if (p == (int)ft_strlen(pattern) && s == (int)ft_strlen(string))
		return (1);
	else
		return (0);
}
